...
/// <summary>
/// Abstract base class for triple-output processors that serialise all incoming signals
/// through a <see cref="SemaphoreSlim"/> before forwarding to three downstream consumers.
/// </summary>
...
/// <typeparam name="TOut3">The type of values forwarded to the third downstream consumer.</typeparam>
public abstract class BCProcessorSyncedI1O3<TIn, TOut1, TOut2, TOut3>
    : BCPartMonitored
    , IBCConsumer<TIn> {
    protected readonly IBCConsumer<TOut1> NextConsumer1;
    protected readonly IBCConsumer<TOut2> NextConsumer2;
    protected readonly IBCConsumer<TOut3> NextConsumer3;
    protected readonly SemaphoreSlim _Semaphore = new(1, 1);

    public BCProcessorSyncedI1O3(
            BCDescription description,
            IBCConsumer<TOut1> nextConsumer1,
            IBCConsumer<TOut2> nextConsumer2,
            IBCConsumer<TOut3> nextConsumer3
        ) : base(
            description
        ) {
        this.NextConsumer1 = nextConsumer1;
        this.NextConsumer2 = nextConsumer2;
        this.NextConsumer3 = nextConsumer3;
    }
    ...
    public virtual async Task OnError(BCError value, CancellationToken cancellationToken) {
        using (this._Monitor?.LogEnter(this, nameof(this.OnError))) {
            ...
            try {
                ...
                await this.NextConsumer3.OnError(value, cancellationToken).ConfigureAwait(false);
            } finally {
    ...
    public virtual async Task OnComplete(CancellationToken cancellationToken) {
        using (this._Monitor?.LogEnter(this, nameof(this.OnComplete))) {
            ...
            if (this.SetCompleted()) {
                ...
                await this.NextConsumer3.OnComplete(cancellationToken).ConfigureAwait(false);
            }
        }
    }
    ...
    public override async Task WaitRightCompletedAsync(CancellationToken cancellationToken) {
        using (this._Monitor?.LogEnter(this, nameof(this.WaitRightCompletedAsync))) {
            ...
            await this.NextConsumer3.WaitRightCompletedAsync(cancellationToken).ConfigureAwait(false);
            ...
            await this.NextConsumer3.WaitSelfCompletedAsync(cancellationToken).ConfigureAwait(false);
        }
    }
    ...
    public override bool SetMonitor(IBCMonitor monitor) {
        ...
        if (result) {
            ...
            monitor.Add(this.NextConsumer3);
        }
        return true;
    }
