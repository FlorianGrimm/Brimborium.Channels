import {Rectangle} from './rectangle'
    ...
    export function ConvexHull(S: Point[]): Point[] {
        let P = S.slice(0).sort((a, b) => a.x !== b.x ? b.x - a.x : b.y - a.y);
        let n = S.length, i;
        let minmin = 0;
        let xmin = P[0].x;
        for (i = 1; i < n; ++i) {
        ...
        if (minmax === n - 1) { // degenerate case: all x-coords == xmin
            ...
            let bot = H.length; // the bottom point of the upper hull stack
            i = maxmin;
            while (--i >= minmax) {
    ...
    function Rtangent_PointPolyC(P: Point, V: Point[]): number {
        ...
        let a: number, b: number, c: number;            // indices for edge chain endpoints
        let upA: boolean, dnC: boolean;           // test for up direction of edges a and c
    ...
    function Ltangent_PointPolyC(P: Point, V: Point[]): number {
        ...
        // use binary search for large convex polygons
        let a: number, b: number, c: number;             // indices for edge chain endpoints
        let dnA: boolean, dnC: boolean;           // test for down direction of edges a and c
    ...
    export function tangent_PolyPolyC(V: Point[], W: Point[], t1: (a: Point, b: Point[]) => number, t2: (a: Point, b: Point[]) => number, cmp1: (a: Point, b: Point, c: Point) => boolean, cmp2: (a: Point, b: Point, c: Point) => boolean): { t1: number; t2: number } {
        let ix1: number, ix2: number;      // search indices for polygons 1 and 2
    ...
    export function LRtangent_PolyPolyC(V: Point[], W: Point[]): { t1: number; t2: number } {
        let rl = RLtangent_PolyPolyC(W, V);
    ...
    export class VisibilityEdge {
        ...
        length(): number {
            let dx = this.source.p.x - this.target.p.x;
            let dy = this.source.p.y - this.target.p.y;
    ...
    export class TangentVisibilityGraph {
        ...
        private intersectsPolys(l: LineSegment, i1: number, i2: number): boolean {
            for (let i = 0, n = this.P.length; i < n; ++i) {
    ...
    function intersects(l: LineSegment, P: Point[]) {
        ...
        for (let i = 1, n = P.length; i < n; ++i) {
            let int = Rectangle.lineIntersection(
                l.x1, l.y1,
                l.x2, l.y2,
                P[i - 1].x, P[i - 1].y,
                P[i].x, P[i].y
    ...
    {
        let m = V.length - 1, n = W.length - 1;
        let bt = new BiTangents();
        for (let i = 0; i <= m; ++i) {
            for (let j = 0; j <= n; ++j) {
                let v1 = V[i == 0 ? m : i - 1];
                let v2 = V[i];
                ...
                let w2 = W[j];
                let w3 = W[j == n ? 0 : j + 1];
                let v1v2w2 = isLeft(v1, v2, w2);
                let v2w1w2 = isLeft(v2, w1, w2);
                let v2w2w3 = isLeft(v2, w2, w3);
                let w1w2v2 = isLeft(w1, w2, v2);
                let w2v1v2 = isLeft(w2, v1, v2);
                let w2v2v3 = isLeft(w2, v2, v3);
