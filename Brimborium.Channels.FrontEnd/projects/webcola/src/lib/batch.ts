 ...
 */
export function gridify(pgLayout: any, nudgeGap: number, margin: number, groupMargin: number) {
...
function route(nodes: any[], groups: any[], margin: number, groupMargin: number) {
    nodes.forEach(d => {
        d.routerNode = <any>{
            name: d.name,
...
export function powerGraphGridLayout(
    ...
    let edges: Array<Link<Node | number>> = [];
    let vs = graph.nodes.slice(0);
    vs.forEach((v, i) => (<any>v).index = i);
    powerGraph.groups.forEach((g:any) => {
        let sourceInd = g.index = g.id + n;
        vs.push(g);
