...
import {separateGraphs, applyPacking} from './handledisconnected'
    ...
    export class Layout {
        ...
        powerGraphGroups(f: Function): this {
            let g = powergraph.getGroups(this._nodes, this._links, this.linkAccessor, this._rootGroup);
        ...
        start(
            ...
            let D = Descent.createSquareMatrix(N, function (i, j) {
                return distances[i][j];
            });

            if (this._rootGroup && typeof this._rootGroup.groups !== 'undefined') {
                let i = n;
                let addAttraction = (i, j, strength, idealDistance) => {
            ...
            let curConstraints = this._constraints || [];
            ...
            if (gridSnapIterations) {
                ...
                let G0 = Descent.createSquareMatrix(N,(i, j) => {
        ...
        private initialLayout(iterations: number, x: number[], y: number[]) {
            if (this._groups.length > 0 && iterations > 0) {
                ...
                let n = this._nodes.length;
                let edges = this._links.map(e => <any>{ source: (<Node>e.source).index, target: (<Node>e.target).index });
                let vs = this._nodes.map(v => <any>{ index: v.index });
        ...
        private separateOverlappingComponents(width: number, height: number, centerGraph: boolean = true): void {
            ...
            if (!this._distanceMatrix && this._handleDisconnected) {
                ...
                this._nodes.forEach(function (v, i) { v.x = x[i], v.y = y[i]; });
                let graphs = separateGraphs(this._nodes, this._links);
        ...
        routeEdge(edge, ah: number = 5, draw) {
            ...
            //    debugger;
            //}
            let vg2 = new TangentVisibilityGraph(this._visibilityGraph.P, { V: this._visibilityGraph.V, E: this._visibilityGraph.E }),
            ...
            let sourceInd = e => e.source.id, targetInd = e => e.target.id, length = e => e.length(),
            ...
            //            let ints = node.innerBounds.lineIntersections(u.x, u.y, v.x, v.y);
            //            if (ints.length > 0) {
            //                debugger;
            //            }
            //        })
