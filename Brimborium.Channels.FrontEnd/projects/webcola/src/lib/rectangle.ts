...
import {Point} from './geom'
    ...
    export class Rectangle {
        ...
        lineIntersections(x1: number, y1: number, x2: number, y2: number): Array<Point> {
            let sides = [[this.x, this.y, this.X, this.y],
                    [this.X, this.y, this.X, this.Y],
                    [this.X, this.Y, this.x, this.Y],
                [this.x, this.Y, this.x, this.y]];
            let intersections = [];
            for (let i = 0; i < 4; ++i) {
                let r = Rectangle.lineIntersection(x1, y1, x2, y2, sides[i][0], sides[i][1], sides[i][2], sides[i][3]);
        ...
        rayIntersection(x2: number, y2: number): Point {
            let ints = this.lineIntersections(this.cx(), this.cy(), x2, y2);
    ...
    export function makeEdgeTo(s: { x: number; y: number }, target: Rectangle, ah: number): Point {
        let ti = target.rayIntersection(s.x, s.y);
    ...
    function generateGroupConstraints(root: ProjectionGroup, f: RectAccessors, minSep: number, isContained: boolean = false): Constraint[]
    {
        ...
        if (gn) root.groups.forEach(g => {
            let b: Rectangle = g.bounds;
            add(f.makeRect(f.getOpen(b), f.getClose(b), f.getCentre(b), f.getSize(b)), g.minVar);
        });
        let cs = generateConstraints(rs, vs, f, minSep);
        if (gn) {
            vs.forEach(v => { v.cOut = [], v.cIn = [] });
            cs.forEach(c => { c.left.cOut.push(c), c.right.cIn.push(c) });
            root.groups.forEach(g => {
                let gapAdjustment = (g.padding - f.getSize(g.bounds)) / 2;
        ...
        rect: RectAccessors, minSep: number): Constraint[]
    {
        ...
        let events = new Array<Event>(N);
        for (i = 0; i < n; ++i) {
            let r = rs[i];
            let v = new Node(vars[i], r, rect.getCentre(r));
            events[i] = new Event(true, v, rect.getOpen(r));
            events[i + n] = new Event(false, v, rect.getClose(r));
        }
        events.sort(compareEvents);
        let cs = new Array<Constraint>();
        let scanline = makeRBTree();
        for (i = 0; i < N; ++i) {
            let e = events[i];
            let v = e.v;
            if (e.isOpen) {
                ...
                // close event
                scanline.remove(v);
                let makeConstraint = (l, r) => {
                    let sep = (rect.getSize(l.r) + rect.getSize(r.r)) / 2 + minSep;
                    cs.push(new Constraint(l.v, r.v, sep));
                };
                let visitNeighbours = (forward, reverse, mkcon) => {
                    let u, it = v[forward].iterator();
    ...
    function findXNeighbours(v: Node, scanline: RBTree<Node>): void {
        let f = (forward, reverse) => {
            let it = scanline.findIter(v);
            let u;
            while ((u = it[forward]()) !== null) {
                let uovervX = u.r.overlapX(v.r);
    ...
    function findYNeighbours(v: Node, scanline: RBTree<Node>): void {
        let f = (forward, reverse) => {
            let u = scanline.findIter(v)[forward]();
    ...
    export class Projection {
            ...
            private avoidOverlaps: boolean = false)
        {
            ...
            if (avoidOverlaps && rootGroup && typeof rootGroup.groups !== 'undefined') {
                nodes.forEach(v => {
                    ...
                    let w2 = v.width / 2, h2 = v.height / 2;
        ...
        private makeFeasible(c: any) {
            ...
            let axis = 'x', dim = 'width';
            if (c.axis === 'x') axis = 'y', dim = 'height';
            let vs: GraphNode[] = c.offsets.map(o => this.nodes[o.node]).sort((a, b) => a[axis] - b[axis]);
            let p: GraphNode = null;
            vs.forEach(v => {
        ...
        private createAlignment(c: any) {
            let u = this.nodes[c.offsets[0].node].variable;
            this.makeFeasible(c);
            let cs = c.axis === 'x' ? this.xConstraints : this.yConstraints;
            c.offsets.slice(1).forEach(o => {
                let v = this.nodes[o.node].variable;
                cs.push(new Constraint(u, v, o.offset, true));
            });
        }

        private createConstraints(constraints: any[]) {
            let isSep = c => typeof c.type === 'undefined' || c.type === 'separation';
        ...
        private setupVariablesAndBounds(x0: number[], y0: number[], desired: number[], getDesired: (v: GraphNode) => number) {
            this.nodes.forEach((v, i) => {
                ...
                let w = (v.width || 0) / 2, h = (v.height || 0) / 2;
                let ix = x0[i], iy = y0[i];
        ...
        xProject(x0: number[], y0: number[], x: number[]) {
            ...
            this.project(x0, y0, x0, x, v=> v.px, this.xConstraints, generateXGroupConstraints,
                ...
                g => {
                    let xmin = x[(<IndexedVariable>g.minVar).index] = g.minVar.position();
                    let xmax = x[(<IndexedVariable>g.maxVar).index] = g.maxVar.position();
                    let p2 = g.padding / 2;
                    g.bounds.x = xmin - p2;
                    g.bounds.X = xmax + p2;
                });
        ...
        yProject(x0: number[], y0: number[], y: number[]) {
            ...
            this.project(x0, y0, y0, y, v=> v.py, this.yConstraints, generateYGroupConstraints,
                ...
                g => {
                    let ymin = y[(<IndexedVariable>g.minVar).index] = g.minVar.position();
                    let ymax = y[(<IndexedVariable>g.maxVar).index] = g.maxVar.position();
                    let p2 = g.padding / 2;
                    g.bounds.y = ymin - p2;;
                    g.bounds.Y = ymax + p2;
                });
