...
import {Calculator} from './shortestpaths'
    ...
    export class LongestCommonSubsequence<T> {
        ...
        constructor(public s: T[], public t: T[]) {
            let mf = LongestCommonSubsequence.findMatch(s, t);
            let tr = t.slice(0).reverse();
            let mr = LongestCommonSubsequence.findMatch(s, tr);
        ...
        private static findMatch<T>(s: T[], t: T[]) {
            let m = s.length;
            let n = t.length;
            let match = { length: 0, si: -1, ti: -1 };
            let l = new Array(m);
            for (let i = 0; i < m; i++) {
                l[i] = new Array(n);
                for (let j = 0; j < n; j++)
                    if (s[i] === t[j]) {
                        let v = l[i][j] = (i === 0 || j === 0) ? 1 : l[i - 1][j - 1] + 1;
                        if (v > match.length) {
                            match.length = v;
                            match.si = i - v + 1;
                            match.ti = j - v + 1;
    ...
    export class GridRouter<Node> {
        ...
        private getGridLines(axis): GridLine[] {
            let columns = [];
            let ls = this.leaves.slice(0, this.leaves.length);
            while (ls.length > 0) {
        ...
        constructor(public originalnodes: Node[], accessor: NodeAccessor<Node>, public groupPadding: number = 12) {
            ...
            let frontToBackGroups = this.backToFront.slice(0).reverse().filter(g=> !g.leaf);
            ...
            let colMids = this.midPoints(this.cols.map(r=> r.pos));
            let rowMids = this.midPoints(this.rows.map(r=> r.pos));

            // setup extents of lines
            let rowx = colMids[0], rowX = colMids[colMids.length - 1];
            let coly = rowMids[0], colY = rowMids[rowMids.length - 1];

            // horizontal lines
            let hlines = this.rows.map(r=> <any>{ x1: rowx, x2: rowX, y1: r.pos, y2: r.pos })
                .concat(rowMids.map(m=> <any>{ x1: rowx, x2: rowX, y1: m, y2: m }));

            // vertical lines
            let vlines = this.cols.map(c=> <any>{ x1: c.pos, x2: c.pos, y1: coly, y2: colY })
                .concat(colMids.map(m=> <any>{ x1: m, x2: m, y1: coly, y2: colY }));

            // the full set of lines
            let lines = hlines.concat(vlines);
            ...
            hlines.forEach(h=>
                vlines.forEach(v=> {
                    let p = new Vert(this.verts.length, v.x1, h.y1);
                    h.verts.push(p);
                    v.verts.push(p);
                    this.verts.push(p);
                    ...
                    while (i-- > 0) {
                        let node = this.backToFront[i],
                            r = node.rect;
                        let dx = Math.abs(p.x - r.cx()),
            ...
            lines.forEach((l, li) => {
                ...
                this.nodes.forEach((v, i) => {
                    v.rect.lineIntersections(l.x1, l.y1, l.x2, l.y2).forEach((intersect, j) => {
                        ...
                        let p = new Vert(this.verts.length, intersect.x, intersect.y, v, l);
                        this.verts.push(p);
                        l.verts.push(p);
                        v.ports.push(p);
                    });
                ...
                let isHoriz = Math.abs(l.y1 - l.y2) < 0.1;
                let delta = (a, b) => isHoriz ? b.x - a.x : b.y - a.y;
                l.verts.sort(delta);
                for (let i = 1; i < l.verts.length; i++) {
                    let u = l.verts[i - 1], v = l.verts[i];
        ...
        private findAncestorPathBetween(a, b) {
            let aa = this.findLineage(a), ba = this.findLineage(b), i = 0;
            while (aa[i] === ba[i]) i++;
        ...
        siblingObstacles(a, b) {
            let path = this.findAncestorPathBetween(a, b);
            let lineageLookup = {};
            path.lineages.forEach(v=> lineageLookup[v.id] = {});
            let obstacles = path.commonAncestor.children.filter(v=> !(v in lineageLookup));
        ...
        static nudgeSegs(x: string, y: string, routes, segments, leftOf, gap: number) {
            ...
            if (n <= 1) return;
            let vs = segments.map(s => new Variable(s[0][x]));
            let cs = [];
            for (let i = 0; i < n; i++) {
                for (let j = 0; j < n; j++) {
                    if (i === j) continue;
        ...
        static nudgeSegments(routes, x: string, y: string, leftOf: (e1: number, e2: number) => boolean, gap: number) {
            let vsegmentsets = GridRouter.getSegmentSets(routes, x, y);
            // scan the grouped (by x) segment sets to find co-linear bundles
            for (let i = 0; i < vsegmentsets.length; i++) {
                let ss = vsegmentsets[i];
                let events = [];
        ...
        routeEdges<Edge>(edges: Edge[], nudgeGap: number, source: (e: Edge) => number, target: (e: Edge) => number): Point[][][] {
            let routePaths = edges.map(e=> this.route(source(e), target(e)));
            let order = GridRouter.orderEdges(routePaths);
            let routes = routePaths.map(function (e) { return GridRouter.makeSegments(e); });
        ...
        static angleBetween2Lines(line1: Point[], line2: Point[]): number {
            let angle1 = Math.atan2(line1[0].y - line1[1].y,
                line1[0].x - line1[1].x);
            let angle2 = Math.atan2(line2[0].y - line2[1].y,
                line2[0].x - line2[1].x);
        ...
        static makeSegments(path: Point[]): Point[][] {
            ...
            let isStraight = (a, b, c) => Math.abs((b.x - a.x) * (c.y - a.y) - (b.y - a.y) * (c.x - a.x)) < 0.001;
            let segments = [];
            let a = copyPoint(path[0]);
            for (let i = 1; i < path.length; i++) {
                let b = copyPoint(path[i]), c = i < path.length - 1 ? path[i + 1] : null;
        ...
        route(s: number, t: number): Point[] {
            ...
            for (let i = 1; i < source.ports.length; i++) {
                let u = source.ports[0].id;
                let v = source.ports[i].id;
            ...
            for (let i = 1; i < target.ports.length; i++) {
                let u = target.ports[0].id;
                let v = target.ports[i].id;
                this.passableEdges.push({
                    source: u,
                    target: v,
                    length: 0
            ...
            let getSource = e=> e.source,
                getTarget = e=> e.target,
                getLength = e=> e.length;

            let shortestPathCalculator = new Calculator(this.verts.length, this.passableEdges, getSource, getTarget, getLength);
            let bendPenalty = (u, v, w) => {
                let a = this.verts[u], b = this.verts[v], c = this.verts[w];
                let dx = Math.abs(c.x - a.x), dy = Math.abs(c.y - a.y);
            ...
            // get shortest path
            let shortestPath = shortestPathCalculator.PathFromNodeToNodeWithPrevCost(
                source.ports[0].id, target.ports[0].id,
                bendPenalty);

            // shortest path is reversed and does not include the target port
            let pathPoints = shortestPath.reverse().map(vi => this.verts[vi]);
            ...
            if (route.length > 1) {
                for (let i = 0; i < route.length; i++) {
                    ...
                    if (i < route.length - 1) {
                        ...
                        let l = route[i + 1];
                        let x0 = l[0].x, y0 = l[0].y;
                        let x1 = l[1].x;
                        let y1 = l[1].y;
                        dx = x1 - x0;
                        dy = y1 - y0;
                        let angle = GridRouter.angleBetween2Lines(li, l) < 0 ? 1 : 0;
                        ...
                        let cx = Math.abs(x2 - x);
                        let cy = Math.abs(y2 - y);
                        result.routepath += 'A ' + cx + ' ' + cy + ' 0 0 ' + angle + ' ' + x2 + ' ' + y2 + ' ';
                    } else {
                        let arrowtip = [x, y];
                        let arrowcorner1, arrowcorner2;
                        if (Math.abs(dx) > 0) {
