    ...
    function intersectionCount(a: number[], b: number[]): number {
        let n = 0;
        for (let i in a) if (typeof b[i] !== 'undefined') ++n;
        return n;
    }

    function getNeighbours<Link>(links: Link[], la: LinkAccessor<Link>): any {
        let neighbours = {};
        let addNeighbours = (u, v) => {
            if (typeof neighbours[u] === 'undefined')
                neighbours[u] = {};
            neighbours[u][v] = {};
        };
        links.forEach(e => {
            let u = la.getSourceIndex(e), v = la.getTargetIndex(e);
            addNeighbours(u, v);
            addNeighbours(v, u);
        });
        return neighbours;
    ...
    function computeLinkLengths<Link>(links: Link[], w: number, f: (a: any, b: any) => number, la: LinkLengthAccessor<Link>) {
        let neighbours = getNeighbours(links, la);
        links.forEach(l => {
            let a = neighbours[la.getSourceIndex(l)];
            let b = neighbours[la.getTargetIndex(l)];
    ...
    {
        let components = stronglyConnectedComponents(n, links, la);
        let nodes = {};
        components.forEach((c,i) =>
            c.forEach(v => nodes[v] = i)
        );
        let constraints: SeparationConstraint[] = [];
        links.forEach(l => {
            let ui = la.getSourceIndex(l), vi = la.getTargetIndex(l),
    ...
    export function stronglyConnectedComponents<Link>(numVertices: number, edges: Link[], la: LinkAccessor<Link>): number[][]{
        ...
        for (let v of nodes) if (typeof v.index === 'undefined') strongConnect(v);
        return components;
    }
