...
export class Calculator<Link> {
    ...
    constructor(public n: number, public es: Link[], getSourceIndex: (l: Link) => number, getTargetIndex: (l: Link) => number, getLength: (l: Link) => number) {
        ...
        let i = this.n; while (i--) this.neighbours[i] = new Node(i);

        i = this.es.length; while (i--) {
            let e = this.es[i];
            let u: number = getSourceIndex(e), v: number = getTargetIndex(e);
            let d = getLength(e);
    ...
    {
        let q = new PriorityQueue<QueueEntry>((a, b) => a.d <= b.d),
        ...
        while(!q.empty()) {
            ...
            let i = u.neighbours.length; while (i--) {
                let neighbour = u.neighbours[i],
                ...
                let viduid = v.id + ',' + u.id;
                if(viduid in visitedFrom && visitedFrom[viduid] <= qu.d)
                    continue;

                let cc = qu.prev ? prevCost(qu.prev.node.id, u.id, v.id) : 0,
    ...
    private dijkstraNeighbours(start: number, dest: number = -1): number[] {
        let q = new PriorityQueue<Node>((a, b) => a.d <= b.d),
            i = this.neighbours.length,
            d: number[] = new Array(i);
        while (i--) {
            let node: Node = this.neighbours[i];
        ...
        while (!q.empty()) {
            ...
            i = u.neighbours.length; while (i--) {
                let neighbour = u.neighbours[i];
                let v = this.neighbours[neighbour.id];
                let t = u.d + neighbour.distance;
