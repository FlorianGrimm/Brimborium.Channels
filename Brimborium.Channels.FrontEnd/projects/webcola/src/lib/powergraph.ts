...
export interface LinkTypeAccessor<Link> extends LinkAccessor<Link> {
    ...
    export class Configuration<Link> {
        ...
        constructor(n: number, edges: Link[], private linkAccessor: LinkTypeAccessor<Link>, rootGroup?: any[]) {
            ...
            this.R = edges.length;
            edges.forEach(e => {
                let s = this.modules[linkAccessor.getSourceIndex(e)],
        ...
        private initModulesFromGroup(group): ModuleSet {
            ...
            for (let i = 0; i < group.leaves.length; ++i) {
                let node = group.leaves[i];
                let module = new Module(node.id);
                this.modules[node.id] = module;
                moduleSet.add(module);
            }
            if (group.groups) {
                for (let j = 0; j < group.groups.length; ++j) {
                    let child = group.groups[j];
        ...
        merge(a: Module, b: Module, k: number = 0): Module {
            let inInt = a.incoming.intersection(b.incoming),
                outInt = a.outgoing.intersection(b.outgoing);
            let children = new ModuleSet();
            children.add(a);
            children.add(b);
            let m = new Module(this.modules.length, outInt, inInt, children);
            this.modules.push(m);
            let update = (s: LinkSets, i: string, o: string) => {
                s.forAll((ms, linktype) => {
                    ms.forAll(n => {
                        let nls = <LinkSets>n[i];
                        nls.add(linktype, m);
        ...
        private rootMerges(k: number = 0): {
            ...
            let rs = this.roots[k].modules();
            let n = rs.length;
            let merges = new Array(n * (n - 1));
            let ctr = 0;
            for (let i = 0, i_ = n - 1; i < i_; ++i) {
                for (let j = i+1; j < n; ++j) {
                    let a = rs[i], b = rs[j];
        ...
        greedyMerge(): boolean {
            for (let i = 0; i < this.roots.length; ++i) {
                ...
                let ms = this.rootMerges(i).sort((a, b) => a.nEdges == b.nEdges ? a.id - b.id : a.nEdges - b.nEdges);
                let m = ms[0];
        ...
        getGroupHierarchy(retargetedEdges: PowerEdge[]): any[]{
            ...
            let es = this.allEdges();
            es.forEach(e => {
                let a = this.modules[e.source];
                let b = this.modules[e.target];
    ...
    function intersection(m: any, n: any): any {
        let i = {};
        for (let v in m) if (v in n) i[v] = m[v];
        return i;
    }
    ...
    export class LinkSets {
        ...
        add(linktype: number, m: Module) {
            let s: ModuleSet = linktype in this.sets ? this.sets[linktype] : this.sets[linktype] = new ModuleSet();
            s.add(m);
            ++this.n;
        }
        remove(linktype: number, m: Module) {
            let ms = <ModuleSet>this.sets[linktype];
            ms.remove(m);
            if (ms.count() === 0) {
        ...
        intersection(other: LinkSets): LinkSets {
            let result: LinkSets = new LinkSets();
            this.forAll((ms, lt) => {
                if (lt in other.sets) {
                    let i = ms.intersection(other.sets[lt]),
                        n = i.count();
                    if (n > 0) {
                        result.sets[lt] = i;
                        result.n += n;
    ...
    export function getGroups<Link>(nodes: any[], links: Link[], la: LinkTypeAccessor<Link>, rootGroup?: any[]): { groups: any[]; powerEdges: PowerEdge[] } {
        ...
        while (c.greedyMerge());
        let powerEdges: PowerEdge[] = [];
        let g = c.getGroupHierarchy(powerEdges);
