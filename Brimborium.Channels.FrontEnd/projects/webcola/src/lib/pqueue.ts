...
export class PairingHeap<T> {
    ...
    public toString(selector) : string {
        let str = "", needComma = false;
        for (let i = 0; i < this.subheaps.length; ++i) {
            let subheap: PairingHeap<T> = this.subheaps[i];
            if (!subheap.elem) {
                needComma = false;
                continue;
            }
    ...
    public mergePairs(lessThan: (a: T, b: T) => boolean) : PairingHeap<T> {
        if (this.subheaps.length == 0) return new PairingHeap<T>(null);
            ...
            let firstPair = this.subheaps.pop().merge(this.subheaps.pop(), lessThan);
            let remaining = this.mergePairs(lessThan);
    ...
    public decreaseKey(subheap: PairingHeap<T>, newValue: T, setHeapNode: (e: T, h: PairingHeap<T>)=>void, lessThan: (a: T, b: T) => boolean): PairingHeap<T> {
        let newHeap = subheap.removeMin(lessThan);
        ...
        let pairingNode = new PairingHeap(newValue);
        if (setHeapNode !== null) {
