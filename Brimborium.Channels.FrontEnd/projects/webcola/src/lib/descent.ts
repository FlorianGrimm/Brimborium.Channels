...
DEBUG */
         ...
         */
        isEmpty(): boolean {
            for (let l in this.locks) return false;
            return true;
        }
        /**
    ...
    export class Descent {
        ...
        constructor(x: number[][], public D: number[][], public G: number[][]= null) {
            ...
            let n = this.n = x[0].length; // number of nodes
        ...
        private offsetDir(): number[] {
            let u = new Array(this.k);
            let l = 0;
            for (let i = 0; i < this.k; ++i) {
                let x = u[i] = this.random.getNextBetween(0.01, 1) - 0.5;
                l += x * x;
            }
            l = Math.sqrt(l);
...
DEBUG */
            ...
            // Grid snap forces
            let r = this.snapGridSize/2;
            let g = this.snapGridSize;
            let w = this.snapStrength;
            let k = w / (r * r);
            let numNodes = this.numGridSnapNodes;
            //let numNodes = n;
            for (let u: number = 0; u < numNodes; ++u) {
                for (i = 0; i < this.k; ++i) {
                    let xiu = this.x[i][u];
                    let m = xiu / g;
                    let f = m % 1;
...
DEBUG */
        ...
        public reduceStress(): number {
            ...
            let alpha = this.computeStepSize(this.g);
            for (let i = 0; i < this.k; ++i) {
...
/* DEBUG
            for (let u: number = 0; u < this.n; ++u)
                for (let i = 0; i < this.k; ++i)
                    if (isNaN(r[i][u])) debugger;
DEBUG */
            if (this.project) {
                this.matrixApply((i, j) => this.e[i][j] = x0[i][j] - r[i][j]);
                let beta = this.computeStepSize(this.e);
        ...
        public run(iterations: number): number {
            let stress = Number.MAX_VALUE, converged = false;
            while (!converged && iterations-- > 0) {
                let s = this.rungeKutta();
        ...
        public rungeKutta(): number {
            ...
            this.matrixApply((i, j) => {
                let x = (this.a[i][j] + 2.0 * this.b[i][j] + 2.0 * this.c[i][j] + this.d[i][j]) / 6.0,
                    d = this.x[i][j] - x;
                disp += d * d;
                this.x[i][j] = x;
            });
        ...
        public computeStress(): number {
            let stress = 0;
            for (let u = 0, nMinus1 = this.n - 1; u < nMinus1; ++u) {
                for (let v = u + 1, n = this.n; v < n; ++v) {
                    let l = 0;
                    for (let i = 0; i < this.k; ++i) {
                        let dx = this.x[i][u] - this.x[i][v];
                        l += dx * dx;
                    }
                    l = Math.sqrt(l);
                    let d = this.D[u][v];
